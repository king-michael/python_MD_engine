...
def test_simulation_LJ_melt(verbose=False):
    ...
    np.testing.assert_array_almost_equal(np.round(force_lammps, 4), np.round(force_pyMD, 4), err_msg='Forces mismatch')
    if verbose:
        print('LAMMPS and pyMD results match')

if __name__ == '__main__':
    test_simulation_LJ_melt()
